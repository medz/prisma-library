...
impl Engine {
    ...
    async fn rollback_transaction(&self, tx_id: String) -> Result<String> {
...
/// Rolls back a transaction with id `tx_id` in the core.
#[no_mangle]
pub extern "C" fn rollback_transaction(
    id: i64,
    tx_id: *const c_char,
    ...
    if let Some(engine) = engine {
        let result = futures::executor::block_on(engine.rollback_transaction(tx_id));
        if result.is_ok() {
